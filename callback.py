...
audioCallbackQueue= queue.Queue(maxsize=32);
def play():
	count=0;
	time.sleep(0.5);
	p = pyaudio.PyAudio()
	s = p.open(format=p.get_format_from_width(2), channels=1, rate=audioRate, output=True,stream_callback=audioCallback,frames_per_buffer=audioChunk//audioDownSample)
	s.start_stream()
	while(1):
		if(aq.empty()):
			time.sleep(0.001);
			...
			audioCallbackQueue.put(auBin);
			count+=1
			print("audio%05d length:%d"%(count,len(au)))

def audioCallback(in_data, frame_count, time_info, status):
	data = audioCallbackQueue.get()
	#print("frame_count=",frame_count,"dataSize",len(data))
